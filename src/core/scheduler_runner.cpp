...
void applyResolved(const Resolved& res, bool ruleChanged) {
    ...
    // Огибающая — единственное, что планировщик двигает на каждом тике:
    // это и есть фейд, и пользователь её напрямую не задаёт.
    st->scheduleEnvelope = res.envelope;

    // Питание и яркость — по той же причине, что и пресет: только в момент
    // смены правила. Иначе слайдер яркости возвращался бы к значению
    // правила раз в секунду, а выключить свет внутри окна было бы нельзя.
